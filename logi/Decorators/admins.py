...
from Drago import DRAGONS, app
from logi.Database import (_get_authusers, add_nonadmin_chat, delete_authuser,
                            get_authuser, get_authuser_count,
                            get_authuser_names, is_nonadmin_chat,
                            remove_nonadmin_chat, save_authuser)
from logi.Utilities.changers import int_to_alpha
    ...
    async def wrapper(_, message):
        ...
        if not is_non_admin:
            ...
            if not member.can_manage_voice_chats:
                if message.from_user.id not in DRAGONS:
    ...
    async def wrapper(_, CallbackQuery):
        ...
        if not is_non_admin:
            ...
            if not a.can_manage_voice_chats:
                if CallbackQuery.from_user.id not in DRAGONS:
