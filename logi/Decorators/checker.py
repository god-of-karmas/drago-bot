from Drago import BOT_USERNAME, EVENT_LOGS, app
from logi.Database import blacklisted_chats, is_gbanned_user, is_on_off
...
def checker(mystic):
    async def wrapper(_, message):
        ...
        if await is_on_off(1):
            if int(message.chat.id) != int(EVENT_LOGS):
                return await message.reply_text(
...
def checkerCB(mystic):
    async def wrapper(_, CallbackQuery):
        ...
        if await is_on_off(1):
            if int(CallbackQuery.message.chat.id) != int(EVENT_LOGS):
                return await CallbackQuery.answer(
