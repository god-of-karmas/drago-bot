...
from Drago import BOT_ID, BOT_USERNAME, BOT_NAME, DRAGONS, app, db_mem
...
from logi.Inline import (add_genre_markup, check_genre_markup, check_markup,
...
async def del_cmd(_, message):
    ...
    if not _playlist:
        await message.reply_text(
            f"You have no Playlist on {BOT_NAME}'s Server"
        )
    else:
        titlex = []
        j = 0
