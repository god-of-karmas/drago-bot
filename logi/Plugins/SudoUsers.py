...
from Drago import BOT_ID, BOT_USERNAME, BOT_NAME, OWNER_ID, DRAGONS, app
from logi.Database import (add_gban_user, add_off, add_on, add_sudo,
                            get_active_chats, get_served_chats, get_DRAGONS,
...
async def useradd(_, message: Message):
    if not message.reply_to_message:
        ...
        if added:
            ...
            os.system(f"kill -9 {os.getpid()} && python3 -m logi")
        else:
            await message.reply_text("Failed")
        return
    if message.reply_to_message.from_user.id in DRAGONS:
        return await message.reply_text(
...
async def userdel(_, message: Message):
    if not message.reply_to_message:
        ...
        if removed:
            await message.reply_text(
                f"Removed **{user.mention}** from {BOT_NAME}'s Sudo."
            )
            return os.system(f"kill -9 {os.getpid()} && python3 -m logi")
    ...
    if user_id not in DRAGONS:
        return await message.reply_text(
            f"Not a part of {BOT_NAME}'s Sudo."
        )
    removed = await remove_sudo(user_id)
    if removed:
        await message.reply_text(
            f"Removed **{mention}** from {BOT_NAME}'s Sudo."
        )
        return os.system(f"kill -9 {os.getpid()} && python3 -m logi")
    await message.reply_text(f"Something wrong happened.")
...
async def DRAGONS_list(_, message: Message):
    DRAGONS = await get_DRAGONS()
    text = "⭐️<u> **Owners:**</u>\n"
    sex = 0
    for x in OWNER_ID:
        try:
        ...
        text += f"{sex}➤ {user}\n"
    smex = 0
    for count, user_id in enumerate(DRAGONS, 1):
...
@app.on_message(filters.command("maintenance") & filters.user(DRAGONS))
...
@app.on_message(filters.command("logger") & filters.user(DRAGONS))
...
@app.on_message(filters.command("gban") & filters.user(DRAGONS))
        ...
        if user.id == from_user.id:
            ...
            for sex in served_chats:
                try:
                    ...
                    pass
            ban_text = f"""
__**New Global Ban on {BOT_NAME}**__
...
@app.on_message(filters.command("ungban") & filters.user(DRAGONS))
...
@app.on_message(filters.command("broadcast_pin") & filters.user(DRAGONS))
...
@app.on_message(filters.command("broadcast_pin_loud") & filters.user(DRAGONS))
...
@app.on_message(filters.command("broadcast") & filters.user(DRAGONS))
...
@app.on_message(filters.command("clean") & filters.user(DRAGONS))
