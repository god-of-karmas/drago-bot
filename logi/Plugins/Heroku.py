...
from config import (HEROKU_API_KEY, HEROKU_APP_NAME, UPSTREAM_BRANCH,
                    UPSTREAM_REPO)
from Drago import EVENT_LOGS, BOT_NAME, DRAGONS, app
...
@app.on_message(filters.command("get_log") & filters.user(DRAGONS))
...
@app.on_message(filters.command("get_var") & filters.user(DRAGONS))
...
@app.on_message(filters.command("del_var") & filters.user(DRAGONS))
...
@app.on_message(filters.command("set_var") & filters.user(DRAGONS))
...
@app.on_message(filters.command("usage") & filters.user(DRAGONS))
...
@app.on_message(filters.command("update") & filters.user(DRAGONS))
...
@app.on_message(filters.command("restart") & filters.user(DRAGONS))
        ...
        try:
            ...
            for x in served_chats:
                try:
                    await app.send_message(
                        x,
                        f"{BOT_NAME} has just restarted herself. Sorry for the issues.\n\nStart playing after 10-15 seconds again.",
                    )
