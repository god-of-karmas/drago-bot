...
from Drago import ASSIDS, BOT_ID, BOT_NAME, OWNER_ID, SUDOERS, app
from Drago import boottime as bot_start_time
from Drago import db, random_assistant
from logi.Core.PyTgCalls import logi
        ...
        try:
            ...
            if member.id in OWNER_ID:
                return await message.reply_text(
                    f"{BOT_NAME}'s Owner[{member.mention}] has just joined your chat."
                )
            if member.id in SUDOERS:
                return await message.reply_text(
                    f"A member of {BOT_NAME}'s Sudo User[{member.mention}] has just joined your chat."
                )
            return
        except:
            return
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "LV":
        ...
        volume = 25
        try:
            await logi.pytgcalls.change_volume_call(c_id, volume)
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"logi Disk Usage: {diske}%", show_alert=True
        )
