...
from Drago import ASSISTANT_PREFIX, DRAGONS, app, random_assistant
from logi.Database import get_assistant, save_assistant
...
@app.on_message(filters.command("changeassistant") & filters.user(DRAGONS))
...
@app.on_message(filters.command("setassistant") & filters.user(DRAGONS))
async def assis_change(_, message: Message):
    ...
    if not _assistant:
        await message.reply_text(
            f"**__ Music Bot Assistant Alloted__**\n\nAssistant No. **{ran_ass}**"
        )
        assis = {
